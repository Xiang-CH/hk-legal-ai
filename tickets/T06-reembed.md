...
- **Scope/files:** `scripts/embed-clic-judgment.mjs` (batch 100 clic / 10 judgment, `text-embedding-3-large`) AND/OR in-DB `azure_openai.create_embeddings`, `scripts/index-output/*.json` (+ `.cached.json` EN cache, `.embeddings.jsonl` sidecars).
- **Steps:**
  1. `node scripts/embed-clic-judgment.mjs [--lang=en]` (judgments always processed): loads `*.cached.json`, reuses embedding ONLY on identical text (`id` match + text compare; edits re-embed), batch sidecars (resumable), merges into fresh JSONs, asserts zero nulls + uniform dims.
